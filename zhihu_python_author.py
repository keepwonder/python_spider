...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'a' and 'data-za-element-name' in attrs_d and 'target' in attrs_d:
            self.a_title = True
        if tag == 'a' and 'author-link' in attrs_d.values():
            self.a_author = True
    ...
    def handle_data(self, data):
        ...
        self.content = dict(zip(self.title, self.author))
        return self.content
...
if __name__ == '__main__':
    ...
    with open('content.csv', 'a+', encoding='utf-8') as csvf:
